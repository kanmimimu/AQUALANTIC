...
object RandomUtils {
    ...
    private fun randomString(length: Int, chars: String): String {
        ...
        for (i in 0 until length) stringBuilder.append(chars[random.nextInt(chars.toCharArray().size)])
        return stringBuilder.toString()
    }
    fun randomUsername(
        maxLength: Int = GuiAltManager.altsLength,
        raw: Boolean = GuiAltManager.unformattedAlts
    ): String {
        if (!GuiAltManager.stylisedAlts)
    ...
    private fun leetRandomly(string: String) = string.mapIndexed { i, char ->
        if (i != 0 && i != string.lastIndex && nextBoolean())
