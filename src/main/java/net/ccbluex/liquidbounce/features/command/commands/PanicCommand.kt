...
class PanicCommand : Command("panic", emptyArray()) {
    ...
    override fun execute(args: Array<String>) {
        ...
        if (args.size > 1 && args[1].isNotEmpty()) {
            when (args[1].lowercase()) {
                ...
                "nonrender" -> {
                    modules = modules.filter { it.category != ModuleCategory.VISUAL }
                    msg = "all non-render"
                }
            ...
            chatSyntax("panic <all/nonrender/combat/player/movement/visual/world/misc/exploit/client/ghost/color>")
            return
        }
    ...
    override fun tabComplete(args: Array<String>): List<String> {
        ...
        return when (args.size) {
            1 -> listOf("all", "nonrender", "combat", "player", "movement", "visual", "world", "misc", "exploit", "ghost", "client", "color")
