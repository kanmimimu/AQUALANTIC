...
class ThemeCommand : Command("theme", arrayOf("thememanager", "tm", "themes")) {
        ...
        if (args.size > 1) {
            when {
                args[1].equals("load", ignoreCase = true) -> {
                    if (args.size > 2) {
                        val themeFile = File(CrossSine.fileManager.themesDir, args[2])
                        ...
                        if (themeFile.exists()) {
                            try {
                                ...
                                CrossSine.hud = Config(theme).toHUD()
                                CrossSine.isStarting = false
                                chat("§6Theme applied successfully.")
                                CrossSine.hud.addNotification(Notification("Theme", "Theme applied successfully.", NotifyType.INFO))
                                playEdit()
                            } catch (e: IOException) {
                                e.printStackTrace()
                            }
                ...
                args[1].equals("save", ignoreCase = true) -> {
                    if (args.size > 2) {
                        ...
                        try {
                            ...
                            val settingsTheme = Config(CrossSine.hud).toJson()
                            chat("§9Saving theme...")
    ...
    private fun getLocalThemes(): Array<File>? = CrossSine.fileManager.themesDir.listFiles()
}
