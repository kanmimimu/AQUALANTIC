    ...
    override fun execute(args: Array<String>) {
        if (args.size == 3) {
            ...
            when (focused.lowercase()) {
                "add" -> {
                    CrossSine.combatManager.focusedPlayerList.add(entity)
                    alert("Successfully added §a${target.lowercase()}§3 into the focus list.")
                    return
                }
                "remove" -> {
                    if (CrossSine.combatManager.focusedPlayerList.contains(entity)) {
                        CrossSine.combatManager.focusedPlayerList.remove(entity)
            ...
            CrossSine.combatManager.focusedPlayerList.clear()
