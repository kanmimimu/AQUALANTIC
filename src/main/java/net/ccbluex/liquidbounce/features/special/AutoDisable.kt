...
object AutoDisable : Listenable {
    ...
    fun onWorld(event: WorldEvent) {
        CrossSine.moduleManager.modules
            ...
            .forEach { module: Module ->
                module.state = false
                CrossSine.hud.addNotification(Notification(this.name, "Disabled ${module.name} due world Changed.", NotifyType.WARNING, 2000))
            }
    }
    ...
    fun onPacket(event: PacketEvent) {
        if (event.packet is S08PacketPlayerPosLook) {
            CrossSine.moduleManager.modules
                ...
                .forEach { module: Module ->
                    module.state = false
                    CrossSine.hud.addNotification(Notification(this.name, "Disabled ${module.name} due flags.", NotifyType.WARNING, 2000))
                }
        }
    }
    ...
    fun handleGameEnd() {
        CrossSine.moduleManager.modules
            ...
            .forEach { module: Module ->
                module.state = false
                CrossSine.hud.addNotification(Notification(this.name, "Disabled ${module.name} due to game end.", NotifyType.WARNING, 2000))
            }
    }
