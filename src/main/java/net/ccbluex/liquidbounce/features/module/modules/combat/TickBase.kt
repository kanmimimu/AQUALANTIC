...
class TickBase : Module(){
    ...
    fun getExtraTicks(): Int {
        ...
        if (!Scaffold.state && timer.hasTimePassed(delayTimer.get().toLong()) && (KillAura.state && KillAura.currentTarget != null && mc.thePlayer.getDistanceToEntity(KillAura.currentTarget) > KillAura.rangeValue.get()) && mc.thePlayer.hurtTime <= 2) {
            counter = ticks.get()
            timer.reset()
            return counter
        }
