...
// Range
    ...
    private var isBlocksmcAutoblocking = false
    ...
    val displayBlocking: Boolean
        get() = (blockingStatus || noEventBlocking || ((autoBlockValue.equals("Fake") || autoBlockValue.equals("WatchDogBlinkLess") || autoBlockValue.equals(
            "WatchDogBlink"
        )) && canFakeBlock) || (autoBlockValue.get().equals("UNCP", true) && blocking)) && canBlock
        ...
        if (!cancelRun && currentTarget != null && mc.thePlayer.getDistanceToEntity(currentTarget!!) <= autoBlockRangeValue.get()) {
            when (autoBlockValue.get().lowercase()) {
                ...
                "blocksmc" -> {
                    if (!canBlock) {
                        if (blinking) {
                            BlinkUtils.setBlinkState(release = true, off = true)
                            blinking = false
                        }
                        return
                    }
                    isBlocksmcAutoblocking = true
                    if (blockTicks >= 3) {
                        blockTicks = 0
                    }
                    ...
                    when (blockTicks) {
                        ...
                        2 -> {
                            ...
                            if (bestSword != -1 && bestSword != mc.thePlayer.inventory.currentItem) {
                                ...
                                mc.thePlayer.inventory.currentItem = bestSword
                                mc.netHandler.addToSendQueue(C09PacketHeldItemChange(bestSword))
                                swapped = true
                            }
                            runAttackLoop(true)
                        }
                        3 -> {
                            if (swapped && originalSlot != -1) {
                                ...
                                mc.netHandler.addToSendQueue(C09PacketHeldItemChange(originalSlot))
                                swapped = false
                                originalSlot = -1
                            }
                            BlinkUtils.setBlinkState(release = true, packetMoving = false)
                            blinking = false
                            startBlockingNoEvent()
                        }
                    }
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is C09PacketHeldItemChange && !isBlocksmcAutoblocking) {
            stopBlocking()
            stopBlockingNoEvent()
        }
    }
    ...
    private fun runAttack(interact: Boolean) {
        ...
        if (hitable) {
            ...
            if (autoBlockValue.get().equals("UNCP", true) && canBlock) {
                mc.netHandler.addToSendQueue(C08PacketPlayerBlockPlacement(mc.thePlayer.inventory.getCurrentItem()))
                this.blocking = true
            }
