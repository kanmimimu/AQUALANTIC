...
import net.ccbluex.liquidbounce.ui.client.gui.clickgui.style.styles.astolfo.AstolfoConstants.BACKGROUND_CATEGORY
import net.ccbluex.liquidbounce.ui.client.gui.clickgui.style.styles.astolfo.AstolfoConstants.FONT
import net.ccbluex.liquidbounce.ui.client.gui.clickgui.style.styles.astolfo.AstolfoConstants.PANEL_HEIGHT
import net.ccbluex.liquidbounce.ui.client.gui.clickgui.style.styles.astolfo.AstolfoConstants.PANEL_WIDTH
...
class AstolfoCategoryPanel(x: Float, y: Float, var category: ModuleCategory, var color: Color) : AstolfoButton(x, y, PANEL_WIDTH, PANEL_HEIGHT) {
  ...
  init {
    val startY = y + height
    for ((count, mod) in CrossSine.moduleManager.modules.filter { it.category.displayName.equals(this.category.displayName, true)}.withIndex()) {
      ClientUtils.logInfo("Added ${mod.name} to ${this.category.displayName}")
