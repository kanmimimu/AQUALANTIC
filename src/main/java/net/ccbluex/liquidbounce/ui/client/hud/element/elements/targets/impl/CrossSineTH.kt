...
class CrossSineTH(inst: TargetHUD) : TargetStyle("CrossSine", inst, true) {
    override fun drawTarget(entity: EntityLivingBase) {
        ...
        val width: Float = if (fonts.getStringWidth(entity.name) > 50F) fonts.getStringWidth(entity.name).toFloat() + 27F else 100F
        ...
        RenderUtils.drawRoundedRect(2F, 27F, 2F + 20F, 29F, 1F, Color(0,0,0,fadeAlpha(180)).rgb)
        RenderUtils.drawRoundedRect(2F, 27F, 2F + (width * easingHealth / entity.maxHealth), 29F, 1F, ClientTheme.getColorWithAlpha(0, fadeAlpha(255)).rgb)
        GlStateManager.disableAlpha()
        GlStateManager.disableBlend()
    }

    override fun getBorder(entity: EntityLivingBase?): Border {
        return Border(0F,0F, if (Fonts.font35.getStringWidth(entity!!.name) > 54F) Fonts.font35.getStringWidth(entity.name).toFloat() + 27F else 104F, 31F)
    }
}