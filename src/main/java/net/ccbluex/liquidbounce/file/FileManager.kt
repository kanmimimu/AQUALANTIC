...
class FileManager : MinecraftInstance() {
    ...
    fun loadConfigs(vararg configs: FileConfig) {
        if (net.ccbluex.liquidbounce.CrossSine.destruced) return
    ...
    private fun saveConfig(config: FileConfig, ignoreStarting: Boolean) {
        if (net.ccbluex.liquidbounce.CrossSine.destruced) return
        if (!ignoreStarting && net.ccbluex.liquidbounce.CrossSine.isStarting) return
        try {
    ...
    fun loadLegacy(): Boolean {
        if (net.ccbluex.liquidbounce.CrossSine.destruced) return false
        var modified = false
        ...
        if (modulesFile.exists()) {
            ...
            try {
                ...
                for ((key, value) in jsonElement.asJsonObject.entrySet()) {
                    val module = net.ccbluex.liquidbounce.CrossSine.moduleManager.getModule(key)
                    if (module != null) {
        ...
        if (macrosFile.exists()) {
            ...
            try {
                ...
                for (jsonElement in jsonArray) {
                    ...
                    net.ccbluex.liquidbounce.CrossSine.macroManager.macros
