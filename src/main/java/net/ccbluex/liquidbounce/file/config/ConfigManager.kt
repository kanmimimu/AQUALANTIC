    ...
    fun loadLegacySupport() {
        ...
        if (oldSettingDir.exists()) {
            oldSettingDir.listFiles()?.forEach {
                ...
                if (!CrossSine.fileManager.legacySettingsDir.exists()) {
                    CrossSine.fileManager.legacySettingsDir.mkdir()
                }

                it.renameTo(File(CrossSine.fileManager.legacySettingsDir, it.name))
            }
